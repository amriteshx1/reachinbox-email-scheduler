...
- Implement a **rate limit** on the number of emails sent per hour:
    - Either **global** (e.g. MAX\_EMAILS\_PER\_HOUR=200), or
    - **Per-sender / per-tenant** (e.g. MAX\_EMAILS\_PER\_HOUR\_PER\_SENDER), you will have to support multiple senders.
...
- Rate-limiting logic must be **safe across multiple workers / instances**:
    - Use **Redis or DB** backed counters (e.g. keyed by hour\_window + sender),
...
**Note**: Do not submit a plagiarized assignment.
