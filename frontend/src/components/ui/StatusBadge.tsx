...
export function StatusBadge({
  ...
  if ((status === "scheduled" || status === "sending") && time) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-badge px-2.5 py-[3px] text-[11px] font-medium text-progress">
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-[3px] text-[11px] font-medium ${styles[status]}`}>
      {labels[status]}
    </span>
  );
}
